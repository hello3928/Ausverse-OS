...
#define IDT_ENTRIES 48      /* 32 exceptions + 16 IRQs */

static struct idt_entry idt[IDT_ENTRIES];
static struct idt_ptr   idtp;

/* IRQ handler table — register your handler with irq_register() */
...
void idt_init(void) {
    ...
    for (int i = 0;  i < 32; i++) idt_set(i,      isr_stubs[i]);
    for (int i = 0;  i < 16; i++) idt_set(i + 32, irq_stubs[i]);
